...
def mensajeEstado(registro):
    ...
    if parametroSoloAnalisis == "si":
        if parametroEstrato == "Manzana" and parametroEncuesta == "ENE":
        ...
        if parametroEstrato == "Manzana" and parametroEncuesta != "ENE":
            if registro.motivo == "Croquis generado":
                ...
                print(s)
                arcpy.AddMessage(s)
                mensaje("Analisis: No se genera croquis")
    ...
    if parametroSoloPlanoUbicacion == "Si":
        ...
        if registro.motivo == "Croquis No generado":
            mensaje("Plano Ubicación: No se logrO generar el croquis Plano Ubicación.")
        ...
        if parametroEstrato == "Manzana" and parametroEncuesta != "ENE":
            if registro.motivo == "Croquis generado":
                ...
                print(s)
                arcpy.AddMessage(s)
                mensaje("Genera croquis: Se genera croquis")
            if registro.motivo == "Croquis No generado":
                s = "#{}#:{},{},{},{},{}".format(registro.codigo, registro.intersectaPE, registro.intersectaCRF, registro.intersectaAV, homologacion, registro.motivo)
                print(s)
                arcpy.AddMessage(s)
                mensaje("Genera croquis: No se genero croquis")
                ...
                if not (registro.estadoViviendas == "Rechazado" or parametroSoloAnalisis == 'si'):
                    ...
                    if mxd != None:
                        ...
                        if preparaMapaManzana(mxd, extent, escala, datosManzana):
                            ...
                            mensaje(rutaPDF)
                            registro.rutaPDF = controlPDF.generaPDF(mxd, rutaPDF, 290)
...
def actualizaVinetaManzanas(mxd,datosManzana):
    ...
    try:
        ...
        #mensaje("titulo = {}".format(infoMarco.titulo))
        ...
        for elm in arcpy.mapping.ListLayoutElements(mxd, "TEXT_ELEMENT"):
            if parametroEncuesta == "ENE":
                ...
                if elm.name == "Nombre_Muestra":
                    elm.text = parametroEncuesta+" "+infoMarco.titulo
    ...
    try:
        ...
        if contenidoCsv == 1:
            ...
            with open(rutaCsv, "wb") as f:
                ...
                a = ['Hora', 'Codigo', 'Estado Proceso', 'Motivo Proceso', 'Estado Superficie','Motivo Superficie','Area Manzana2016 (m2)','Area Manzana2017 (m2)','Estado Viviendas','Motivo Viviendas','CUT', 'CODIGO DISTRITO', 'CODIGO LOCALIDAD O ZONA', 'CODIGO ENTIDAD', 'Ruta PDF', 'Intersecta PE', 'Intersecta CRF', 'Intersecta AV', 'Homologacion', 'Formato / Orientacion', 'Escala', "Codigo barra"]
                wr.writerow(a)
                for r in registros:
                    cut, dis, area, loc, ent = descomponeManzent(r.codigo)
                    a = [r.hora, r.codigo, r.estado, r.motivo, r.estadoSuperficie, r.motivoSuperficie, round(r.area_manzana2016,2), round(r.area_manzana2017,2), r.estadoViviendas, r.motivoViviendas, cut, dis, loc, ent, r.rutaPDF, r.intersectaPE, r.intersectaCRF, r.intersectaAV, r.homologacion.encode('utf8'), r.formato +" / "+ r.orientacion, r.escala, r.codigoBarra.encode('utf8')]
                    wr.writerow(a)
